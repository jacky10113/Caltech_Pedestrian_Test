classdef BaseZoom < handle
    ...
    methods
            ...
            switch this.axesClass
                ...
                case 'figure'
                    if this.YAxis.number == 1
                        ...
                        %disp(this.affinePosition);
                        %disp(this.XAxis.scale);
                        %disp(this.YAxis.(this.direction).scale);
        ...
        function affinePosition = get.affinePosition(this)
            ...
            if any(~isfinite(this.XLimNew(:))) || any(~isfinite(this.YLimNew(:)))
                error('Invalid values in XLimNew or YLimNew.');
            end
            ...
            if any(~isfinite(tmp1)) || any(~isfinite(tmp2))
                error('Invalid values returned by transformCoordinate.');
            end
            ...
            if affinePosition(1, 3) <= 0 || affinePosition(1, 4) <= 0
                error('Invalid affinePosition: width and height must be positive.');
            end
        end
