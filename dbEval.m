function dbEval(expsIndex,bottomSymbol)
...
pth.annDir=[pth.rootDir 'test_annotations_txt'];
...
% plot curves and bbs
% Predefined list of 11 colors
 colors = [
        70/255, 153/255, 144/255;  % Dark green/Teal
        60/255, 180/255, 75/255;   % Green
        255/255, 225/255, 25/255;  % Yellow
        67/255, 99/255, 216/255;   % Blue
        245/255, 130/255, 49/255;  % Orange
        145/255, 30/255, 180/255;  % Purple
        66/255, 212/255, 244/255;  % Cyan
        240/255, 32/255, 230/255;  % Pink
 ...
 elseif (expsIndex==3),hTitle='Performance Comparison on Small Subset';
 elseif (expsIndex==10),hTitle='Performance Comparison on Occlusion=Heavy Subset';    
 end
     
     
 plotExps( res, plotRoc, plotAlg, plotNum, pth.evalDir, ...
   samples, lims, colors, {algs.style}, {algs.type},hTitle,bottomSymbol);
 % samples, lims, reshape([algs.color]',3,[])', {algs.style}, {algs.type},hTitle);
...
function plotExps( res, plotRoc, plotAlg, plotNum, plotName, ...
  samples, lims, colors, styles, algtypes,hTitle,bottomSymbol)
...
%  hTitle   - Title for the plot
%  bottomSymbol -Bottom label (a),(b)(c)...
...
%     kp=ord==kp(1)|ord==kp(2); j=find(cumsum(~kp)>=plotNum-2); 
  ...
  % Set the background color of the entire graphics window to white
  set(gcf, 'Color', 'w'); 
  clf; grid on; hold on; n=length(xs1); h=zeros(1,n);
  for i=1:n, h(i)=plot(xs1{i},ys1{i},'Color',colors1(i,:),...
      'LineStyle',styles1{i},'LineWidth',1.5); end
  
  if( plotRoc )
    yt=[.05 .1:.1:.5 .64 .8];
    ytStr = arrayfun(@(x) sprintf('%02d', round(x*100)), yt, 'UniformOutput', false);
    %ytStr=int2str2(yt*100,2);
    ...
    xlabel({'false positives per image',bottomSymbol},'FontSize', 20, 'FontName', 'Times New Roman');
    ylabel('miss rate','FontSize', 20, 'FontName', 'Times New Roman'); axis(lims);
    plotTile = title(hTitle, 'FontSize', 23, 'FontName', 'Times New Roman');
  ...
  % Set legend border color and line thickness 
  set(lgd, 'EdgeColor', [0.5 0.5 0.5]);  
 
  lgd.ItemTokenSize = [10, 1];  % Adjust the length of the sample line
 
  set(gca, 'LineWidth', 2,'FontName', 'Times New Roman', 'FontSize', 20,'Box', 'on');
...
function numbersArray = extractNumbersFromFilenames(directoryPath, setNumber, vNumber)
    % Read the txt file in pubictest_annotations to obtain the number after I, which is used to read the test results
    % input parameter:
    %   directoryPath - The directory path containing txt files
    %   setNumber - The number after the set
    %   vNumber - The number after V
    % Output parameters:
    %   numbersArray - An array composed of extracted numbers

    fileList = dir(fullfile(directoryPath, '*.txt'));

    numbersArray = [];

    pattern = sprintf('set%02d_V%03d_I(\\d+)\\.txt', setNumber, vNumber);
 ...
 pLoad={'lbls', lbls, 'ilbls', ilbls,...
  ...
  for s=1:length(setIds), s1=setIds(s);
    for v=1:length(vidIds{s}), v1=vidIds{s}(v);
      ...
      %因为自己定义的测试数据集的图片编号取值范围为[0,A.nFrame-1],不是固定的skip
      for f=0:1:A.nFrame-1
        %annName=sprintf('%/set%02d_V%03d_I%05d.txt', s1, v1, f);
        annName=sprintf('%s/set%02d_V%03d_I%05d.txt', pth.annDir, s1, v1, f);
        if exist(annName, 'file') == 2
...
for i=1:nAlg
  ...
  for s=1:length(setIds), s1=setIds(s);
    for v=1:length(vidIds{s}), v1=vidIds{s}(v);
      A=AS{s,v}; 
      %frames=skip-1:skip:A.nFrame-1;  
      vName=sprintf('%s/set%02d/V%03d',aDir,s1,v1);
      frames=extractNumbersFromFilenames(pth.annDir,s1,v1) 
